...
## Root-Level Agents (Custom — Take Priority)

These are customized agents with proper model routing, memory, and command bindings. They override any subdirectory agent with overlapping scope.

| Agent | Model | Purpose | Invoked By |
|-------|-------|---------|------------|
| `architect.md` | Opus | Architecture planning (read-only) | `/architect` |
| `security-auditor.md` | Opus | OWASP security review | `/security-audit` |
| `api-architect.md` | Opus | OpenAPI 3.1 spec design | `/api-spec` |
| `code-reviewer.md` | Sonnet | Code review + quality gates | `/code-review`, `/security-check` |
| `backend-dev.md` | Sonnet | Route/Service/Repository implementation | `/api-endpoint`, `/build` |
| `frontend-dev.md` | Sonnet | Component-based UI with design tokens | `/build-page` |
| `tdd-test-writer.md` | Sonnet | RED phase only (failing tests) | `/tdd` |
| `doc-verifier.md` | Haiku | Verify library APIs via Context7 | `/verify-docs` |

## Design Agents (Upgraded)

| Agent | Model | Purpose | Invoked By | Memory |
|-------|-------|---------|------------|--------|
| `design/ui-designer.md` | Sonnet | UI design, components, design systems | `/design-review`, `/build-page` | user |
| `design/ux-researcher.md` | Opus | User research, personas, journey maps | `/ux-research` | user |
| `design/brand-guardian.md` | Opus | Brand identity, tokens, consistency | `/brand-audit` | user |
| `design/visual-storyteller.md` | Sonnet | Infographics, presentations, data viz | `/visual-story` | user |
| `design/whimsy-injector.md` | Haiku | Micro-interactions, delight, easter eggs | `/whimsy` | user |

## Subdirectory Agents (Contains Studio Template)

These came from the Contains Studio template. They provide broader coverage (marketing, operations) but use generic prompts. Root-level agents always win when both could handle a task.
