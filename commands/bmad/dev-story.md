...
3. **Load story:**
   - If story document exists: Read `docs/{project-name}/stories/STORY-{ID}.md` (new format), fall back to `docs/stories/STORY-{ID}.md` (legacy)
   - Else: Find story in sprint plan `docs/*/sprint-plan.md` (new format), fall back to `docs/sprint-plan-*.md` (legacy)
   - If legacy found, log: "Legacy format detected. Consider moving to `docs/{name}/` subfolder"
4. **Load architecture** (if Level 2+): Read `docs/*/architecture.md` (new format), fall back to `docs/architecture-*.md` (legacy)
