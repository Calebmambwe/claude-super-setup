...
3. **Verify architecture exists:**
   - Check for: `docs/*/architecture.md` (new format), fall back to `docs/architecture-*.md` (legacy)
...
4. **Load requirements document:**
   - Check for PRD: `docs/*/prd.md` (new format), fall back to `docs/prd-*.md` (legacy)
   - If no PRD, check for tech-spec: `docs/*/tech-spec.md` (new format), fall back to `docs/tech-spec-*.md` (legacy)
   - If legacy found, log: "Legacy format detected. Consider moving to `docs/{name}/` subfolder"
...
1. Read architecture document from `docs/*/architecture.md` (or legacy `docs/architecture-*.md`)
...
**Save report:**
- Path: `{output_folder}/{project-name}/solutioning-gate-check.md`
- Use Write tool
